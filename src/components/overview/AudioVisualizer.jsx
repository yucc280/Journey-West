...
export default function AudioVisualizer({ audioSrc = '/media/music.mp3' }) {
    ...
    const render = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        if (barHeight > 0) {
          ctx.beginPath();
          if (ctx.roundRect) {
            ctx.roundRect(x, height - barHeight, Math.max(barWidth - 3, 2), barHeight, [4, 4, 0, 0]);
          } else {
            ctx.rect(x, height - barHeight, Math.max(barWidth - 3, 2), barHeight);
          }
          ctx.fill();
        }
    ...
    <div className="audio-visualizer-card">
      <div className="audio-header">
        <div>
          ...
          <span className="audio-subtitle">点击播放主题曲音律与实时频谱可视化</span>
        </div>
